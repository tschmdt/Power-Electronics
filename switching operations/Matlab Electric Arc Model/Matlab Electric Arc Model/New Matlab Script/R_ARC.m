%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Simulation if an AC arc in series with a resistor
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
F =@(x)x(2)*x(3)*x(1)./(x(3)*x(1).*atan(x(4).*x(1))+x(2));
...
m=Te/Tau;
for k=2:N   
    I(k)=-V(k-1)/R+Vg(k-1)/R;
    V(k)=1/(1+m)*(m*F([I(k), a, Rc, b])+V(k-1)); 
end

%Plot results
figure,
title('Arc in series with a resistor')
xlabel('Time [s]')
hold on
